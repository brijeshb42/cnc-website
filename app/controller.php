...
class Controller {
	...
	function __construct() {
		/*$var = json_decode(getenv("VCAP_SERVICES"),true);
		$db = $var["mysql-5.1"][0]["credentials"];*/
		//new DB\SQL($f3->get('db'),$db['username'],$db['password'])
		$f3=Base::instance();
		$this->db = $f3->set('DB',new DB\SQL($f3->get('db'),'cnc','cncadmin'));
